...
def get_final_preds(model, tokenizer, dset, embedings, labels, text_template='{}', temp_scaling=None, batch_size=128,
    ...
    for j in  range(len(embedings)) :
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            text_probs = (100.0 * embedings[j] @ text_features.T).softmax(dim=-1)
        vals, indics = text_probs.max(dim=1)
        predictions = np.append(predictions, indics.cpu().numpy())
        actual = np.append(actual, labels[j].numpy())
...
def get_preds(model, tokenizer, dset, text_template='{}', temp_scaling=None, batch_size=128,
    ...
    for image, labels in dataloader:
        ...
        vals, indics = text_probs.max(dim=1)
        predictions = np.append(predictions, indics.cpu().numpy())
        img_embedings = np.append(img_embedings, image_features.cpu().numpy())
        actual = np.append(actual, labels.numpy())
        probs = np.append(probs, vals.cpu().numpy())

    """    
    #below will append paticular row of logits and label    
    ...
    softmaxes = torch.nn.functional.softmax(logits_list, dim=1)
    confidences, predictions = torch.max(softmaxes, 1)
    #it compares the predictions with val_labels to get a tensor of accuracies.
    #calculate here accuracy between image and text
    accuracies = predictions.eq(labels_list).int()"""
    return predictions, actual, img_embedings, probs
