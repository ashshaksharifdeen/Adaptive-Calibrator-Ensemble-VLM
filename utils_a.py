...
def sample_calibration_set(predictions_val, actual_val, img_embedings_val, probs_val , ratio=0, seed=333):
    np.random.seed(seed)
    if ratio == 0:
        #return img_embedings_val.numpy(), actual_val.numpy()
        return  probs_val, actual_val 
    ...
    accuracies = predictions_val.eq(actual_val).int()

    index = np.array(range(len(actual_val)))
    #These lines create lists of indices where the predictions are correct (index_t) and incorrect (index_p).
    index_t = [id for id in range(len(actual_val)) if accuracies[id]==1]
    index_p = [id for id in range(len(actual_val)) if accuracies[id]==0]
    #This calculates and prints the original ratio of correct samples to incorrect samples in the dataset.
    ratio_org  = len(index_t)/len(index_p)
    print('Original ratio of correct samples and incorrect samples is: %4f' %(ratio_org))
    #These lines calculate the number of correct (n_t) and incorrect (n_p) samples.
    n_t = len(index_t)
    n_p = len(index_p)
    if n_t > n_p:
        n_sample = int(n_p*ratio)
        #n_t: This is the number of correct samples, which determines the range of indices from which to sample.
        #n_sample: This is the number of samples to draw from the range of indices.
        #randomly selecting specific samples index from range of indexes 
        samples = np.random.choice(n_t, n_sample, replace=True)
        """
        np.array(index_t) would be [1, 3, 5, 7, 9].
        np.array(index_t)[samples] would select elements from index_t at indices specified in samples, so it would be [1, 5, 5, 9].
        Therefore, in this example, sample_index_t would be [1, 5, 5, 9]
        """
        #selecting the specific range of index values of correct
        sample_index_t = np.array(index_t)[samples]
        #selecting the full range of index values of false
        sample_index_p = np.array(index_p)
    else:
        n_sample = int(n_t/ratio)
        samples = np.random.choice(n_p, n_sample, replace=True)
        #selecting the full range of index values of correct
        sample_index_t = np.array(index_t)
        #selecting the specific range of index values of false
        sample_index_p = np.array(index_p)[samples]
    # ipdb.set_trace()
    sample_index = np.concatenate((sample_index_t, sample_index_p), axis=0)
    np.random.shuffle(sample_index)
    #selecting the range of logit and label values according to sample index. here instead of labels select text values.
    #think how to select image logit and text logit acording to indexes
    sampled_logits = probs_val[sample_index]
    sampled_labels = actual_val[sample_index]
    return sampled_logits, sampled_labels

def sample_calibration_set_size(val_logits, val_labels, r=1.):
    np.random.seed(333)
    # if ratio == 0:
    #     return val_logits.numpy(), val_labels.numpy()
    softmaxes = torch.nn.functional.softmax(val_logits, dim=1)
    confidences, predictions = torch.max(softmaxes, 1)
    accuracies = predictions.eq(val_labels).int()

    index = np.array(range(len(val_labels)))
    index_t = [id for id in range(len(val_labels)) if accuracies[id]==1]
    index_p = [id for id in range(len(val_labels)) if accuracies[id]==0]

    n_t = len(index_t)
    n_p = len(index_p)
    n_sample = int(n_t*r)
    print('correct samples: {}'.format(n_sample))
    sample_index_t = np.array(index_t)[:n_sample]

    n_sample = int(n_p*r)
    print('incorrect samples: {}'.format(n_sample))
    sample_index_p = np.array(index_p)[:n_sample]
    # ipdb.set_trace()
    sample_index = np.concatenate((sample_index_t, sample_index_p), axis=0)
    np.random.shuffle(sample_index)
    sampled_logits = val_logits.numpy()[sample_index]
    sampled_labels = val_labels.numpy()[sample_index]
    return sampled_logits, sampled_labels

def imagenet_r_valid_labels():
...
if __name__ == '__main__':
    ...
    (y_probs_val, y_val), (y_probs_test, y_test) = unpickle_probs('/home/ycliu/zouyl/uncertainty/logits/probs_lenet5_c10_logits.p', True)

    y_probs_val = torch.from_numpy(y_probs_val)
    y_val = torch.from_numpy(np.squeeze(y_val))
    logits, labels = sample_calibration_set(y_probs_val, y_val, 1.0)